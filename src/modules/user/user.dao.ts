...
class UserDao implements IUserDao {
    ...
    public async createUser(u: AddUserDto): Promise<MessageLayerDtoT<User>> {
        ...
        await this.docClient.put({ TableName: TableName.User, Item: newUser }).promise();
    ...
    public async getByKey(id: string, publicAddress: string): Promise<MessageLayerDtoT<User>> {
        const result = await this.docClient.get({ TableName: TableName.User, Key: { id, publicAddress } }).promise();
